...
import { UserRole, ExpertStatus } from "../types/index.js";
...
async function clearSeedData(): Promise<void> {
  const experts = await Expert.find({ mobile: { $in: SEED_PHONES } }).select("userId");
  const expertUserIds = experts.map((e) => e.userId);

  await Expert.deleteMany({ mobile: { $in: SEED_PHONES } });
...
async function seedAdmin() {
  const admin = await User.findOneAndUpdate(
    { phone: "9000000001" },
    {
      ...
      avatar: generateDummyAvatar("admin"),
      role: UserRole.ADMIN,
      isVerified: true,
      isBlocked: false,
      walletBalance: 0,
  ...
  for (const u of USERS) {
    const doc = await User.findOneAndUpdate(
      { phone: u.phone },
      {
        ...
        name: u.name,
        email: u.email,
        avatar: generateDummyAvatar(u.phone),
        role: UserRole.USER,
        isVerified: true,
        isBlocked: false,
...
async function seedExperts(categoryIds: mongoose.Types.ObjectId[]) {
  ...
  for (const exp of EXPERTS) {
    let user = await User.findOne({ phone: exp.mobile });
    if (!user) {
      user = await User.create({
        phone: exp.mobile,
        name: exp.name,
        avatar: generateDummyAvatar(exp.mobile),
        role: exp.isApproved ? UserRole.EXPERT : UserRole.USER,
      ...
      user.name = exp.name;
      user.role = exp.isApproved ? UserRole.EXPERT : UserRole.USER;
