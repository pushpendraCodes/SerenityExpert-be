...
async function createExpertByAdmin(data) {
    const existingExpert = await Expert_js_1.default.findOne({ mobile: data.mobile });
    if (existingExpert)
        throw new AppError_js_1.ConflictError("Expert with this mobile number already exists");
    const existingUser = await User_js_1.default.findOne({ phone: data.mobile });
    if (existingUser)
    ...
    const user = await User_js_1.default.create({
        phone: data.mobile,
        name: data.name,
        avatar: (0, constants_js_1.generateDummyAvatar)(data.mobile),
        isVerified: false,
...
async function findExpertByMobile(mobile) {
    return Expert_js_1.default.findOne({ mobile });
}
async function assertExpertCanLogin(mobile) {
    const expert = await Expert_js_1.default.findOne({ mobile });
    if (!expert) {
...
async function getExpertReviews(expertId, limit = 10) {
    const expert = await Expert_js_1.default.findOne({ _id: expertId, isApproved: true });
    if (!expert)
        throw new AppError_js_1.NotFoundError("Expert");
    const reviews = await Call_js_1.default.find({
        expertId,
